...
public class BuckyGlow extends PApplet {

 // import serial port library
 // import library for dialog boxes
...
int c; // current color clicked
int [] cV = new int[11]; // color stored on dodecahedron
int [] cRecent = new int[10]; // colors stored for recent colors
...
int buttonStoreBaseColor,buttonStoreBaseHighlight,buttonStoreShapeColor, buttonStoreShapeHighlight, buttonRunBaseColor, buttonRunBaseHighlight, buttonRunShapeColor, buttonRunShapeHighlight;
...
float speedRaw;
float speedPercent;
float minSpeedPosition = 316.614f; // position of left side of speed slide bar
float maxSpeedPosition = 414.327f-316.614f; // position of right side of speed slide bar
float maxSpeed = 100; // maximum speed that patterns can be played
float minSpeed = 1.5f;  // minimum speed that patterns can be played
...
public void setup() {
  ...
  if(firstRun){ // RUN this only once, so that if program is restarted user does not have to reselect their Serial port
     ...
     i = PApplet.parseInt(COMx.toLowerCase())-1; // Index of serial port that user selected
  ...
  pent3[0]=new PVector(273.5f, 183);
  pent3[1]=new PVector(311, 169);
  pent3[2]=new PVector(362, 253.5f);
  pent3[3]=new PVector(271, 293.5f);
  pent3[4]=new PVector(247.5f, 265);
  ...
  pent5[0]=new PVector(129, 182.5f);
  pent5[1]=new PVector(90, 171.5f);
  pent5[2]=new PVector(40, 258);
  pent5[3]=new PVector(130, 294.5f);
  pent5[4]=new PVector(155, 263);
  
  pent6[0]=new PVector(86.5f,166);
  pent6[1]=new PVector(96,69.5f);
  pent6[2]=new PVector(47,115);
  pent6[3]=new PVector(25,180.5f);
...
public void draw() {
   ...
   image(colorwheel, 0.69f*figSizeX,8,figSizeX*0.28f,figSizeX*0.28f); // display colorwheel
   ...
   textFont(font);
   fill(0); //text color
   text("SELECTED COLOR",0.73f*figSizeX,0.4f*figSizeX);
   
   textFont(font2);
   text("TURN LED OFF:",0.725f*figSizeX,0.32f*figSizeX);

   textFont(font2);
   text("RECENT COLORS:",0.69f*figSizeX,0.515f*figSizeX);
   
   strokeWeight(1); // Black box to turn off LED
   rect(0.88f*figSizeX, 0.298f*figSizeX, boxSize*0.5f, boxSize*0.5f);
   ...
   strokeWeight(1.5f);
   stroke(0);
   rect(0.81f*figSizeX, 0.41f*figSizeX, boxSize, boxSize); // Selected color box
   ...
   // END DISPLAY STORED IMAGES
    ...
    // BUTTONS /////////////////////////////////////////////////////
     ...
     textFont(font3);
     text("RESET",buttonX3-19, buttonY3+5.5f); // display RESET text on reset button and color according to whether or not mouse is over button
     // END RESET BUTTON ////////////////
     ...
     textFont(font3);
     text("SAVE",buttonX4-16, buttonY4+5.5f);   // display SAVE text on reset button and color according to whether or not mouse is over button 
     // END SAVE BUTTON ////////////////
     ...
     speed=PApplet.parseInt(speedRaw*8000); // speed pattern is played to send to Arduino if play is clicked. Value from scroll bar needs to be adjusted so that range is between min and max speed
     
     hs1.update();
     hs1.display();
     textFont(font3);
     fill(0);
     text("SPEED: "+PApplet.parseInt(speedPercent)+"%",274,480); 
...
public boolean containsPoint(PVector[] verts, float px, float py) {
...
//}
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "BuckyGlow" };
    if (passedArgs != null) {
